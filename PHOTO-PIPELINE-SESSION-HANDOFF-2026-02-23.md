...
- NAS mount reliability/workability addressed:
  - CIFS mount options include `uid=1000,gid=1000,file_mode=0664,dir_mode=0775,noperm`.
...
3. Monitor with: `photo-sync-status` and log tail.

## Verification commands
```bash
photo-sync-status
tail -n 40 ~/tools/logs/photo-archive-$(date +%F).log
